...
class TinyTwi {
   private:
    ...
    static void onReceiveService(uint8_t address, int numBytes);
    static bool onCheckAddressService(int);

   public:
    TinyTwi();
    void begin();
    void begin(uint8_t id, uint8_t I2C_SLAVE_ADDR);
    void begin(uint8_t I2C_SLAVE_ADDR) {
        this->begin(0, I2C_SLAVE_ADDR);
    }
    uint8_t read();
    uint8_t receive();
    uint8_t available();
    ...
    void end();
    void onReceive(void (*)(uint8_t, int));
