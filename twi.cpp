...
static bool (*twi_checkAddresToReciver)(int);
...
void Twi_attachCheckAddress(bool (*function)(int)) {
    twi_checkAddresToReciver = function;
}
...
ISR(USI_START_VECTOR) {
    ...
    if (!(PIN_USI & (1 << PIN_USI_SDA))) {
        ...
        if (currently_receiving) {  // if we had received something, this is a RESTART condition and we have
            ...
            twi_onSlaveReceive(rxAddress, rxByteNum);
...
ISR(USI_OVERFLOW_VECTOR) {
    switch (overflowState) {
        ...
        case USI_SLAVE_CHECK_ADDRESS:
            if ((USIDR == 0) || (twi_checkAddresToReciver && twi_checkAddresToReciver(USIDR >> 1))) {
                rxAddress = (USIDR >> 1);
                if (USIDR & 0x01) {
                    overflowState = USI_SLAVE_SEND_DATA;
                    twi_onSlaveTransmit(rxAddress);  // call user event function for a request from master
                } else {
    ...
    if ((PIN_USI & (1 << PIN_USI_SDA)) && (PIN_USI & (1 << PIN_USI_SCL))) {
        ...
        if ((PIN_USI & (1 << PIN_USI_SDA)) && (PIN_USI & (1 << PIN_USI_SCL))) {
            ...
            if (currently_receiving) {                                        // If we are receiving bytes from a master, call user callback
                if (rxByteNum > 0) twi_onSlaveReceive(rxAddress, rxByteNum);  // check, if anything is in the rx buffer, because it is possible,
